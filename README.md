...
- **Neon + Prisma P1002 / advisory lock timeout on deploy:** Pooler hostnames (`-pooler` in the URL) often cannot complete `migrate deploy`. The prestart script uses a **direct** connection for migrations only: it auto-derives the host by dropping `-pooler` from `DATABASE_URL`, or you can set **`DIRECT_DATABASE_URL`** explicitly in Render (Neon dashboard → “Connection details” → direct). Runtime traffic should keep using the pooled `DATABASE_URL`.
