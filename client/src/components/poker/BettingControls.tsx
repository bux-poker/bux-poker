...
export function BettingControls({ 
  ...
  // Calculate button width to match main action buttons
  const buttonWidth = '120px'; // Same width for all main buttons

  return (
    <div className="flex flex-col items-end gap-3">
      {/* Main Action Buttons - Right justified, same size */}
      <div className="flex items-center gap-3" style={{ width: `calc(${buttonWidth} * 3 + 0.75rem * 2)` }}>
        <button
          onClick={() => onAction("FOLD", 0)}
          className="rounded-lg bg-red-600 px-6 py-3 text-base font-bold text-white shadow-lg hover:bg-red-700 transition-colors flex-1"
        ...
        <button
          ...
          className="rounded-lg bg-blue-600 px-6 py-3 text-base font-bold text-white shadow-lg hover:bg-blue-700 transition-colors flex-1"
          style={{ minWidth: buttonWidth }}
        >
        ...
        <button
          ...
          className="rounded-lg bg-emerald-600 px-6 py-3 text-base font-bold text-white shadow-lg hover:bg-emerald-700 transition-colors flex-1"
          style={{ minWidth: buttonWidth }}
        >
          RAISE {raiseAmount}
        </button>
      ...
      {/* Preset Buttons - Right justified, stacked vertically */}
      <div className="flex flex-col gap-2" style={{ width: `calc(${buttonWidth} * 3 + 0.75rem * 2)` }}>
        {/* Top row: 1/2 and POT */}
        <div className="flex items-center gap-3">
          ...
          {/* Spacer to match 3-button width */}
      ...
      {/* Amount Input with +/- Controls - Right justified, same height as both preset rows combined */}
      <div className="flex items-center gap-2" style={{ width: `calc(${buttonWidth} * 3 + 0.75rem * 2)` }}>
        <button
        ...
        <input
          type="number"
          className="h-[68px] flex-1 rounded-lg border-2 border-slate-600 bg-slate-800 px-4 text-center text-lg font-bold text-white focus:border-emerald-500 focus:outline-none"
          style={{ 
