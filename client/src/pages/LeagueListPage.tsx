    ...
    <div className="space-y-6 py-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">
          Active leagues — table, schedule, and links to each leg.
        </p>
      </div>
      {leagues.length === 0 ? (
        <p className="text-slate-400">No active leagues right now.</p>
      ) : (
          ...
          {leagues.map((L) => (
            <li key={L.id}>
              ...
              >
                ...
                <p className="mt-2 text-xs text-slate-500">{L.totalGames} games</p>
              </Link>
            </li>
          ))}
        </ul>
